...
public class AppMenu {
     ...
     * 2. Crea cadena de dependencias (DAOs → Services) mediante createMascotaService()
     * 3. Crea MenuHandler con Scanner y MascotaService
     * 4. Setea running=true para iniciar el loop
     *
     * Patrón de inyección de dependencias (DI) manual:
     * - MicrochipDAO (sin dependencias)
     * - MascotaDAO (depende de MicrochipDAO)
     * - MicrochipServiceImpl (depende de MicrochipDAO)
     * - MascotaServiceImpl (depende de MascotaDAO y MicrochipServiceImpl)
     * - MenuHandler (depende de Scanner y MascotaServiceImpl)
     *
     ...
     * 1  → Crear Mascota (con microchip opcional)
     * 2  → Listar mascotas (todas o filtradas)
     * 3  → Actualizar mascota
     * 4  → Eliminar mascota (soft delete)
     * 5  → Crear microchip independiente
     * 6  → Listar microchip
     * 7  → Actualizar microchip por ID 
     * 8  → Eliminar microchip por ID (PELIGROSO - puede dejar FKs huérfanas)
     * 9  → Actualizar microchip de una mascota 
     * 10 → Eliminar microchip de una mascota (SEGURO - actualiza FK primero)
     ...
     * 1. MicrochipDAO: Sin dependencias, acceso directo a BD
     * 2. MascotaDAO: Depende de MicrochipDAO (inyectado en constructor)
     * 3. MicrochipServiceImpl: Depende de MicrochipDAO
     * 4. MascotaServiceImpl: Depende de MascotaDAO y MicrochipServiceImpl
     *
     * Arquitectura resultante (4 capas):
     * Main (AppMenu, MenuHandler)
     *   ↓
     * Service (MascotaServiceImpl, MicrochipServiceImpl)
     *   ↓
     * DAO (MascotaDAO, MicrochipDAO)
     *   ↓
     ...
     * - Para insertar/actualizar microchips al crear/actualizar mascotas
     * - Para eliminar microchips de forma segura (eliminarMicrochipDeMascota)
     *
     * Patrón: Factory Method para construcción de dependencias
     *
     * @return MascotaServiceImpl completamente inicializado con todas sus dependencias
     */
